...
def metrics(y_true, probability):
    ...
    pr_auc = average_precision_score(y_true, probability) if np.any(y_true == 1) else np.nan
    return {
...
predictions = {"logistic": np.zeros(len(data)), "random_forest": np.zeros(len(data)), "pytorch_mlp": np.zeros(len(data))}
forest_importances = []
...
metrics_table.to_csv(TABLES / "model_metrics.csv", index=False)

importance_table = pd.DataFrame({
    "feature": FEATURES,
    "importance": np.mean(forest_importances, axis=0),
}).sort_values("importance", ascending=False)
importance_table.to_csv(TABLES / "random_forest_feature_importance.csv", index=False)
...
for bird, subset in pred_table.groupby("bird_id"):
    ...
    for model_name in predictions:
        ...
        per_bird.append(row)
pd.DataFrame(per_bird).to_csv(TABLES / "per_bird_metrics.csv", index=False)
